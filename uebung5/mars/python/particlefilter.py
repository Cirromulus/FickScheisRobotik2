...
def apply_motion_model_(wheels):
    ...
    if math.fabs(c) < 0.01:
        ...
        g = 0.485584*(wheels[0]+wheels[1])/(2*c)
        cos1 = np.cos(dResult)
        sin1 = np.sin(dResult)
        cos2 = np.cos(population[:,2])
        sin2 = np.sin(population[:,2])
        np.subtract(sin1, sin2, sin1)
        np.subtract(cos1, cos2, cos1)
        np.multiply(sin1, g, sin1)
        np.multiply(cos1, g, cos1)
        np.add(population[:,0], sin1, population[:,0])
        np.add(population[:,1], cos1, population[:,1])
